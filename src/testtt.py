...
def run(gpu):
    ...
    for epoch in range(epochs):
        ...
        for data, target in train_set:
            ...
            optimizer.step()
        logging.info(
            "Epoch {} Loss {:.6f} Global batch size {} on {} ranks".format(
                epoch, epoch_loss / num_batches, gbatch_size, dist.get_world_size()
            )
        )
    if gpu:
        logging.info(
            "GPU training time= {}".format(str(datetime.datetime.now() - time_start))
        )
    else:
        logging.info(
            "CPU training time= {}".format(str(datetime.datetime.now() - time_start))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", action="store_true", help="Use GPU and CUDA")
    ...
    dist.init_process_group(
        backend="gloo", init_method="env://", world_size=int(world_size), rank=int(rank)
    )
    run(gpu=True)
